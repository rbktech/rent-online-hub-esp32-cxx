    ...
    private:
        ...
        void onResult(BLEAdvertisedDevice advertisedDevice) override
        {
            ...
            if(nameDevice == BLE_CONNECT_NAME) {
                ...
                CSettings::getInstance()->mMacLock = address;

                mBle->mDevice = new BLEAdvertisedDevice(advertisedDevice);

                mBle->mExpected = false;

                Serial.println("Find device: " + String(nameDevice.data()) +
                    " mac: " + String(CSettings::getInstance()->mMacLock.data()));
    ...
    */
    ...
    static void notifyCallback(BLERemoteCharacteristic* pCharacteristic, uint8_t* pData, size_t length, bool isNotify)
    {
        ...
        if((char)pData[0] == '{' && (char)pData[1] == '%' && (char)pData[length - 1] == '}') {
            ...
            doc[0]["id_house"] = String(stg->mId.data());
            doc[0]["address_house"] = "ул. , д. , кв. ";
            doc[0]["command"] = "set_key";
            doc[0]["message"] = sendData;
            doc[0]["bridge"] = String(stg->mMacBridge.data());
            doc[0]["lock"] = String(stg->mMacLock.data());
            ...
            if(value.length() > 0) {
                ...
                if(item_command_settings == "settings") {
                    ...
                    if(item_ip_address_00 != "null") {
                        stg->SERV[0] = item_ip_address_00.toInt();
                        EEPROM.write(EEPROM_ADDRESS_IP_0, stg->SERV[0]);
                    }
                    ...
                    if(item_ip_address_01 != "null") {
                        stg->SERV[1] = item_ip_address_01.toInt();
                        EEPROM.write(EEPROM_ADDRESS_IP_1, stg->SERV[1]);
                    }
                    ...
                    if(item_ip_address_02 != "null") {
                        stg->SERV[2] = item_ip_address_02.toInt();
                        EEPROM.write(EEPROM_ADDRESS_IP_2, stg->SERV[2]);
                    }
                    ...
                    if(item_ip_address_03 != "null") {
                        stg->SERV[3] = item_ip_address_03.toInt();
                        EEPROM.write(EEPROM_ADDRESS_IP_3, stg->SERV[3]);
                    }

                    String item_port = doc[0]["port"];
                    Serial.println("ble server: port: " + item_port);
                    if(item_port != "null") {
                        stg->PORT = item_port.toInt();
                        EEPROM.write(EEPROM_ADDRESS_PORT_0, stg->PORT);
                        EEPROM.write(EEPROM_ADDRESS_PORT_1, stg->PORT >> 8);
                    }
                    ...
                    if(item_wifi_name != "null") {
                        ...
                        memset(stg->ssid, 0, SIZE_WIFI_PARAM);

                        unsigned size = item_wifi_name.length();
                        if(size < SIZE_WIFI_PARAM)
                            for(int i = 0; i < size; i++)
                                stg->ssid[i] = item_wifi_name[i];

                        for(int i = 0; i < SIZE_WIFI_PARAM; i++)
                            EEPROM.write(EEPROM_ADDRESS_SSID + i, stg->ssid[i]);
                    }
                    ...
                    if(item_wifi_password != "null") {
                        ...
                        memset(stg->pass, 0, SIZE_WIFI_PARAM);

                        unsigned size = item_wifi_password.length();
                        if(size < SIZE_WIFI_PARAM)
                            for(int i = 0; i < size; i++)
                                stg->pass[i] = item_wifi_password[i];

                        for(int i = 0; i < SIZE_WIFI_PARAM; i++)
                            EEPROM.write(EEPROM_ADDRESS_PASS + i, stg->pass[i]);
                    }
                ...
                if(item_id != String(CSettings::getInstance()->mId.data()))
                    return;
