...
void setup()
{
    ...
    CDataExchange::getInstance()->getDevice(WIFE)->init();
    CDataExchange::getInstance()->getDevice(WIFE)->connect();
...
void loop()
{
    try {
        ...
        if(CDataExchange::getInstance()->emptyListCommand() == true) {
            ...
            CWiFe* obj = (CWiFe*)(CDataExchange::getInstance()->getDevice(WIFE));

            size = obj->receive(buffer, size);
            if(size != 0) {
                // CPrint::printMessage("int client: recv:", recvData.c_str(), size);
                printf("int client: recv: %s", buffer);
                CDataExchange::getInstance()->parseMessage(buffer);
            }
