...
struct TDataExchange {
    String command;
    String result;
    uint8_t message1[50] = { 0 };
    uint8_t message2[50] = { 0 };
};
...
private:
    ...
    bool mDataExchange = false;

    WiFiClient* mClient = nullptr;
    byte mBuffer[SIZE_BUFFER] = { 0 };
    ...
    void connectClient()
    {
        ...
        if(mClient->connect(SERVER, PORT) == true)
...
public:
    CWiFi()
    {
        ...
        gMacBridge = WiFi.macAddress();
        Serial.print("\tmac: " + gMacBridge);
        Serial.println("\tsuccess");
    }
